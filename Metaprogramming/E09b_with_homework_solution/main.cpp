...
main()
{
    ...
    // In the current implementation we use the transform function to pass the elements to a lambda. This has the
    // inherent problem that the lambda is unable to distinguish whether the element type is a value or a lvalue
    // reference.
    //
    // Hint: add a tuple_element_t alias using the at function from Metaprogramming.h. Instead of using transform,
    // create a similar function that executes the predicate on the tuple_element_t at each index instead of using the
    // decltype of the passed element.
    ...
    // Hint: This is similar to the push_back operation from Metaprogramming.h, only difference is that we "push_back"
    // the elements of the second tuple, not the whole tuple
    ...
    // Note: some filter tests may start failing after updating tuple_cat to correctly handle references. This is
    // expected as the current filter implementation implicitly assumes that tuple_cat strips references from the
    // element type. You may want to temporarily comment out the failing filter tests. We will re-enable them in step 5
    ...
    // Homework step5:
    // In order to make filter work correctly with reference type elements, introduce a tuple_result_t metafunction in
    // the detail namespace earlier changes to execute predicate on correct type. Hint: you can use a similar approach
    // as in the current filter implementation when building the result type: Wrapping element types that pass the
    // filter into a tuple type and inserting empty tuples for those that don't (use if_ from Metaprogramming.h). Then
    // pass that type to the tuple_cat_result_t metafunction created in step 2.
    Tester::test("filter_result", []() {
        static_assert(std::is_same_v<detail::filter_result_t<Tuple<int, double, unsigned>, std::is_integral>,
                                     Tuple<int, unsigned>>);
        static_assert(std::is_same_v<detail::filter_result_t<Tuple<int, int &, int &&>, std::is_reference>,
                                     Tuple<int &, int &&>>);
    });
